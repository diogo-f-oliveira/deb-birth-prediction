...
Existing module entry points use `experiments/gp_full_par.json` and `experiments/nn_full_par.json`, preserving the prior example hyperparameters. They train and report validation metrics; held-out test evaluation is separate. These examples are not tiny checks and do not reproduce the archived paper settings exactly. Production boundary training is explicitly rejected until the fixed-offset training implementations exist. T04 subsequently selected gplearn and verified separate normalized/boundary GP prototypes; see [the decision and experiment](gp_backend_decision.md). Production new-model/artifact interfaces remain T05/T06. No common training CLI is included.
